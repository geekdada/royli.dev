export const metadata = {
  id: 'eda6abbf-99e6-491a-822e-b280e20736de',
  title: '借由 Litestream 方便备份你的 SQLite 数据库',
  slug: 'use-litestream-to-duplicate-sqlite-databases',
  excerpt: '只需极低的成本就可以实现 SQLite 的数据安全储存。',
...
[Litestream](https://litestream.io/)  是一个用 Go 编写的，用于 SQLite 数据库的流式复制工具。它作为一个单独的后台进程运行，并不断地从磁盘上复制写前日志页到一个或多个副本。这种异步复制提供了类似于 Postgres 或 MySQL 等数据库服务器的灾难恢复。
...
<Callout icon="💡">本文以 Ubuntu 系统为例</Callout>


为了避免本文随时间推移而过时，请参照 [官方文档](https://litestream.io/install/debian/) 进行安装。安装完成后 Litestream 便会以守护程序的方式随系统启动。
...
- 关于配置文件完整的解释请看 [这里](https://litestream.io/reference/config/)。

- [如何创建一个 S3 储存库并安全地存取](https://litestream.io/guides/s3/)
...
- [文档](https://litestream.io/reference/restore/)
